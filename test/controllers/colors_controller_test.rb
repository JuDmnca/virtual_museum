...
class ColorsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create color" do
    assert_difference('Color.count') do
      post colors_url, params: { color: { color: @color.color, history: @color.history, image: @color.image, symb: @color.symb } }
    end
  ...
  test "should update color" do
    patch color_url(@color), params: { color: { color: @color.color, history: @color.history, image: @color.image, symb: @color.symb } }
